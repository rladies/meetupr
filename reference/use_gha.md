...
- jwt:
  ...
  Name of the GitHub secret that contains the JWT token. Defaults to
  "MEETUPR_JWT_TOKEN".

- client_key:

  Name of the GitHub secret that contains the client id. Defaults to
  "MEETUPR_client_key".
...
- token_path:
  ...
  Path to the encrypted token file in the repo. Defaults to
  get_encrypted_path().
...
- use_gha_encrypted_token(): For regular OAuth apps. This creates a
  ...
  refreshes it when needed, and writes the rotated encrypted file back
  to the repo. Requires a decryption password stored as a repository
  secret (default: MEETUPR_ENCRYPT_PWD).
...
- `use_gha_encrypted_token()`: Create workflow for encrypted token
  rotation.
