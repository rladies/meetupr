...
rladies <- meetup_groups[
  which(grepl("r-ladies", tolower(meetup_groups$name))),
]
rladies
```
...
```{r growth}
df <- rladies_pro |>
  count(Date = as.Date(founded_date)) |>
  complete(
...
```{r no_recent_events}
rladies |>
  filter(last_event < as.POSIXct("2019-03-29"), is.na(next_event)) |>
  arrange(last_event)
```
