...
S7::method(execute, MeetupQuery) <- function(
  ...
  if (show_progress) {
    cli::cli_progress_bar(
      format = "Fetching data {cli::pb_spin} Page {page_count}, {length(all_data)} items",
      format_done = "Fetched {length(all_data)} items in {page_count} page{?s}",
      total = NA
    )
  }
...
create_event_query <- function(template, max_results = NULL) {
  create_meetup_query(
    template = template,
    pageinfo_path = "data.groupByUrlname.events.pageInfo",
...
create_rsvp_query <- function(template) {
  create_meetup_query(
    template = template,
    pageinfo_path = "data.event.rsvps.pageInfo",
...
create_groups_search_query <- function(max_results) {
  create_meetup_query(
    ...
    pageinfo_path = "data.groupSearch.pageInfo",
...
create_pro_query <- function(
  ...
  create_meetup_query(
    template = template,
    pageinfo_path = glue::glue("data.proNetwork.{network_type}.pageInfo"),
    edges_path = glue::glue("data.proNetwork.{network_type}.edges"),
    total_path = glue::glue(
      "data.proNetwork.{network_type}.{if (network_type == 'eventsSearch') 'totalCount' else 'count'}"
    ),
