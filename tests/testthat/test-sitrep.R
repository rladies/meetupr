...
test_that("meetup_sitrep runs with no auth configured", {
  withr::local_envvar(
    MEETUP_CLIENT_ID = "",
  ...
  local_mocked_bindings(
    ...
    token_path = function(...) stop("No token found")
  )

  result <- meetup_sitrep()

  expect_equal(result$active_method, "None")
})
...
test_that("check_auth_methods detects OAuth not authenticated", {
  withr::local_envvar(
    MEETUP_CLIENT_ID = "test_client",
    MEETUP_CLIENT_SECRET = "test_secret",
    MEETUP_TOKEN = "",
  ...
  expect_equal(result$active_method, "OAuth (not authenticated)")
})
...
test_that("display_auth_status shows CI mode", {
  auth_status <- list(
    active_method = "OAuth",
    oauth = list(
      ci_mode = TRUE,
